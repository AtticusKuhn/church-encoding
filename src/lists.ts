import { and, cBool, False, If, not, True } from './booleans'
import { cPair, first, pair, second } from './pairs'
import { lt } from './predicates'
import { add, pred, sub, succ, zero } from './nat'

// Now lists are really cool.
// There are a few ways to implement them, this is how I've done it

// ## Basics
type cList<T> = (cPair<cBool, cBool>) | (cPair<cBool, cList<T>>);
...
export const nil: cList = pair<cBool, cBool>(True)(True)
...
// ```
export const isNil: (x: cList) => cBool = first
...
export const cons = <T>(a: T) => (b: cList) => pair<cBool, cList>(False)(pair(a)(b))
